...
def handle_client(
    client_socket: socket.socket, client_address: str, replication_info: ReplicationInfo
):
...
def handle_master_conn(
    ...
    while True:
        incoming_message = master_socket.recv(128)
        ...
        pprint(f"Received from master replication command: {data_decoded}")
...
def start_redis_server():
    ...
    if args.replicaof is not None:
        ...
        master_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # tcp over ipv4
        ...
        master_socket.sendall(encode_resp(["REPLCONF", "listening-port", str(args.port)]).encode())
    ...
    replication_info.__master = master_info
...
if __name__ == "__main__":
    ...
    # print(decode_resp(data=data, start_index=151))
